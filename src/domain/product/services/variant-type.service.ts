...
export class VariantTypeService {
  ...
  async findById(id: string): Promise<DomainVariantTypeEntity> {
    return await this.variantTypeRepository.findById(id);
  }
  async findAll(): Promise<DomainVariantTypeEntity[]> {
    return await this.variantTypeRepository.findAll();
  }
  async findWithPagination(query: {
    ...
    return this.variantTypeRepository.findPagination(query);
  }
  ...
  async update(
    ...
    const isExit = await this.variantTypeRepository.findById(id);
    if (!isExit) {
